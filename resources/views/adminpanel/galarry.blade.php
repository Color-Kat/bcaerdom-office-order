...
@section('content')
                                ...
                                <form method="post" enctype='multipart/form-data'>
                                    ...
                                    <select name="id">
                                        @foreach($promis as $pr)
                                            <option value="{{$pr->id}}">{{$pr->name_typedeal}}-{{$pr->areaMax}}
                                                -crmId:{{$pr->crmId}} </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <div>
                                        @if (isset($gallery))
                                            @foreach($gallery as $gal)
                                                <div><img src="/public/images/gallery/{{$gal->image}}" width="80%"> <a
                                                        href="/del/?id={{$gal->id_gallery}}">Удалить</a></div>
                                            @endforeach
                                        @endif
                                    </div>
    ...
    <script>
        function next(e) {
            ...
            } else e.nextElementSibling.style.display = 'block';
        }
    </script>
